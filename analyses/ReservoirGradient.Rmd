...
```{r fig.width=4, fig.height=4,echo=FALSE,fig.cap="Water Chemistry"}
library(png)
library(grid)
...
plot(UL.sim$bray.mean ~ UL.sim$distance, col= "black", bg = cols, pch=mol, las = 1,
  ...
  legend("topright", legend = levels(UL.sim$molecule), pch=c(21, 24), 
       pt.bg = c("gray15", "gray75"), bty='n', cex = 1.25)
...
plot(UL.sim2$DNA ~ UL.sim$distance, col= "black", bg = cols, pch=mol, las = 1,
  ...
  legend("topleft", legend = levels(UL.sim$molecule), pch=c(21, 24), 
       pt.bg = c("gray15", "gray75"), bty='n', cex = 1.25)
