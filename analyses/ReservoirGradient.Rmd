...
traj.animate <- data.frame(scores(ul.pcoa)) %>% 
  ...
  mutate(molecule = ifelse(molecule == "DNA", "Total", "Active")) %>% 
  mutate(transect = desc(distance)) %>% 
  ggplot(aes(x = Dim1, y = Dim2, frame = transect, cumulative = F)) +
  geom_point(alpha = 0.75, size = 10, aes(color = molecule)) + 
  scale_color_manual(values = my.cols) +
  theme(legend.title = element_blank()) +
  coord_fixed() +
...
soil.core.mods <- apply(in.lake.core.from.soils.REL, MARGIN = 2, FUN = function(x) summary(lm(x ~ design.dna$distance))$coefficients[2,c(1,4)])
...
plot_grid(BP.plot + theme(axis.title.x = element_blank()), 
  ...
  ggsave("../figures/05_ecosystem-functions.pdf", bg = "white", width = 7, height = 10)
```
