...
lake %>% 
  ...
  labs(x = (expression("Soil" %->% "Stream" %->% "Reservoir" %->% "Dam (m)")),
       y = "Observed Richness") +
...
traj.animate <- data.frame(scores(ul.pcoa)) %>% 
  ...
  geom_point(alpha = 0.75, size = 3, aes(color = molecule)) + 
  geom_path(alpha = 0.75, size = 1, arrow = arrow(angle = 20,
                          length = unit(0.35, "cm"),
                          type = "closed"), aes(color = molecule)) +
...
gganimate(traj.animate, filename = "../figures/02_trajectory-animation.mp4", ani.width = 800, ani.height = 800)
```
...
UL.sim %>% 
  ...
  labs(y = "Mean Community Similarity to Soils", 
       x = (expression("Soil" %->% "Stream" %->% "Reservoir" %->% "Dam (m)"))) + 
  scale_color_manual("Community Subset", values = my.cols) + 
  scale_fill_manual("Community Subset", values = my.cols) +
  scale_x_reverse(limits = c(400,0)) + 
