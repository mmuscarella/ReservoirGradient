...
main.map <- ggplot(aes(long,lat), data = env.dat) + 
  geom_polygon(fill = "skyblue", alpha = 0.7, size = 0.5, color = 'black', data = ul, alpha = 1) +
...
inset <- inset + geom_map(data = usa, map = usa,
                    ...
                    color = "black", fill = "#e7e7e7", alpha = 0.8, size = 0.15)

inset <- inset + geom_map(data = IN, map = IN,
                    aes(x = long, y=lat, map_id = region),
                    color = "black", fill = map.colors[80], alpha = 0.8, size = 0.15)

inset <- inset + geom_point(aes(x = -86.503087, y = 39.188686), color = "black", shape = 18, size = 2)
...
ggiNEXT(divestim) + 
  ...
  ggsave("../figures/rareextrapcurves.png", dpi = 500, 
         width = 7, height = 7/1.618, units = "in", scale = 1.2)
...
hill.estim %>% filter(type == "water") %>% 
  ...
  ggplot(aes(x = distance, y = Estimator, color = molecule, fill = molecule, shape = molecule)) + 
  geom_point(size =3) + 
  ...
  ggsave("../figures/alpha-diversity.png", dpi = 500, width = 7, height = 1.618*7) + 
  ggsave("../figures/alpha-diversity.pdf", bg = "white", width = 7, height = 1.618*7)
...
data.frame(scores(ul.pcoa)) %>% 
  ...
  geom_point(alpha = 0.5, aes(color = molecule, shape = molecule)) + 
...
UL.sim %>% 
  ...
  ggplot(aes(x = distance, y = bray.mean, color = molecule, fill = molecule, shape = molecule)) +
...
otus.fold.change <- combined.relabunds / combined.relabunds$max_soil_relabund # Calculate fold changes
