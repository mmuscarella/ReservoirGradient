...
hill.estim %>% filter(type == "water") %>% 
  ...
  ggsave("../figures/alpha-diversity.png", dpi = 500, width = 7, height = 10) + 
  ggsave("../figures/alpha-diversity.pdf", bg = "white", width = 7, height = 10)
...
core.soil.taxa.soil.max <- as.matrix(apply(OTUsREL[which(design$type == "soil"), rownames(core.soil.taxa.DNA.REL.bounds)], MARGIN = 2, max))
...
env.dat %>% select(dist.dam, DO, pH, TP, color, chla) %>% 
  ...
  ggplot(aes(x = dist.dam, y = value, color = variable)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
