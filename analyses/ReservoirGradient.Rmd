...
plot(N.phyto ~ design.dna$distance)
...
UL.sim %>% 
  ...
  scale_fill_manual("Community Subset", values = my.cols) +
  scale_x_reverse() +
  annotate(geom = "text", x = 100, y = 0.2, size = 6,
...
UL.sim2 %>% 
  ggplot(aes(x = distance, y = DNA, color = molecule)) +
  geom_point() + 
  geom_smooth(method = "lm") + 
...
UL.sim2 %>% 
  ggplot(aes(x = distance, y = RNA, color = molecule)) +
  geom_point() + 
  geom_smooth(method = "lm") + 
...
BP.plot <- ggplot(metab, aes(x = dist, y = BP)) + 
  ...
  annotate("text", x = 50, y = 1.5, size = 5, label = "paste(italic(R)^2, \" = 0.39\")", parse = T ) + 
...
BR.plot <- ggplot(metab, aes(x = dist, y = BR)) + 
  ...
  annotate("text", x = 50, y = 1.5, size = 5, label = "paste(italic(R)^2, \" = 0.75\")", parse = T ) +
...
BGE.plot <- ggplot(metab, aes(x = dist, y = BGE)) + 
  geom_point() + 
  geom_smooth(method = "lm", formula = y ~ x + I(x^2), color = "black") +
  annotate("text", x = 50, y = .6, size = 5, label = "paste(italic(R)^2, \" = 0.23\")", parse = T ) +
  labs(y = "BGE", 
...
plot_grid(BP.plot + theme(axis.title.x = element_blank()), 
  ...
  ggsave("../figures/04_ecosystem-functions.pdf", bg = "white", width = 6, height = 10)
...
data.frame(transients = terr.REL, dist = design.dna$distance) %>% 
  left_join(metab) %>% gather(metab, value, -transients, -dist) %>% 
