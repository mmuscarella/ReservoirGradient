...
text(40, 1.8, labels = bquote(r^2 == .(BP.R2)), cex = 1.5)
...
text(40, 3.75, labels = bquote(r^2 == .(BR.R2)), cex = 1.5)
...
text(40, 0.535, labels = bquote(r^2 == .(BGE.R2)), cex = 1.5)
...
text(40, 1500, labels = bquote(R^2 == .(D1.R2)), cex = 1)
...
legend("topright", legend = levels(lake$molecule), pch=c(22, 24), 
       pt.bg = c("gray15", "gray75"), bty='n', cex = 1.25, ncol=2)
...
plot(UL.sim$bray.mean ~ UL.sim$distance, col= "black", bg = cols, pch=mol, 
     las = 1, xlim = c(400, 0), ylim = c(0, 0.25), cex = 1.5, 
     xlab="", ylab="", xaxt="n")
...
text(40, 0.125, labels = bquote(R^2 == .(D2.R2)), cex = 1)
...
text(40, 3, labels = bquote(r^2 == .(T1.R2)), cex = 1)
...
text(40, -1, labels = bquote(r^2 == .(T2.R2)), cex = 1)
...
par(mfrow = c(1,1), mar = c(5, 5, 1, 1) + 0.5)

plot(core.REL ~ design.dna$distance, col= colors()[290], pch=22, las = 1, cex = 1.5, 
     xlab="", ylab="", xlim = c(400, 0), ylim = c(0, 0.3), bg = colors()[290])
  ...
  col.range <- c(330, 310, 285, 261)
  col.index <- 1
  for(i in c(0.60, 0.70, 0.80, 0.90)){
    lake.core.i <- lake.rna[, which((colSums(lake.rna.pa) / 
                                   nrow(lake.rna.pa)) >= i)]
    core.total.i <- w.dna[, colnames(lake.core.i)]
    core.rich.i <- rowSums((core.total.i > 0) * 1)
    core.REL.i <- rowSums(core.total.i) / rowSums(w.dna) 
    
    points(core.REL.i ~ design.dna$distance, col = colors()[col.range[col.index]], pch = 22,
           las = 1, cex = 1.5)
    col.index <- col.index + 1
  }
  
  legend("topright", c(".60", ".70", ".75", ".80", ".90"), 
         col = c(colors()[330], colors()[310], colors()[290], colors()[285], colors()[261]), 
         pt.bg = c("white", "white", colors()[290], "white", "white"),
         pch = 22, cex = 1)
  
  mtext("Distance (m)" , side = 1, line = 3, cex=1.5)
  mtext("Core Relative Abundance", side = 2, line = 4, cex=1.5)
