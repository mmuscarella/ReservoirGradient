...
fit.t.mm <- nls(t.rel.abund ~ mm((400-distance), max, halfsat), data = terrestrial, start = list(max = 0.75, halfsat = (400-325)), trace = T)
...
fit.l.mm <- nls(l.rel.abund ~ mm((400-distance), max, halfsat), data = lake, start = list(max = 0.20, halfsat = (400-300)), trace = T)
...
plot(in.lake.core.soil.REL ~ design.dna$distance, 
     ylim = c(0, 1), xlim = c(400, 0), pch = 22, bg = "black", 
...
legend("topright", c("Terrestrial", "Lake"), col = c("black", "black"),
         pt.bg = c("black", "white"), pch = 22, cex = 1, bty = "n")
...
core.soil <- as.data.frame(cbind(core.soil.taxa$Class, core.soil.taxa$Order, core.soil.taxa.DNA.REL.bounds))
colnames(core.soil) <- c("Class", "Order", "DNA.min", "DNA.max", "RNA.min", "RNA.max")
core.water <- as.data.frame(cbind(core.water.taxa$Class, core.water.taxa$Order, core.water.taxa.DNA.REL.bounds))
