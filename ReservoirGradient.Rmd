...
# define the 'core' taxa as otus present in 80% of samples
...
bind_rows(in.soil.to.plot, in.lake.to.plot) %>% 
  ...
  geom_line(alpha = 0.5, stat = "smooth", method = "lm", se = F, show.legend = F) +
  scale_y_log10() +
  scale_x_reverse() + 
...
```{r sig_taxa, fig.asp=1/.618}
as.data.frame(OTUsREL[,nonsoil.core.increasing$OTU]) %>% 
...
as.data.frame(OTUsREL[,soil.core.increasing$OTU]) %>% 
  rownames_to_column("sampleID") %>% 
  left_join(rownames_to_column(design, "sampleID")) %>% 
  gather(OTU, rel_abund, -station, -molecule, -type, -distance, -sampleID) %>% 
  filter(molecule == "DNA") %>% left_join(OTU.tax) %>% 
...
as.data.frame(OTUsREL[,soil.core.decreasing$OTU]) %>% 
  rownames_to_column("sampleID") %>% 
  left_join(rownames_to_column(design, "sampleID")) %>% 
  gather(OTU, rel_abund, -station, -molecule, -type, -distance, -sampleID) %>% 
  filter(molecule == "DNA") %>% left_join(OTU.tax) %>% 
