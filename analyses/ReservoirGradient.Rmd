...
lake %>% 
  ...
  annotate(geom = "text", x = 50, y = 1500, size = 6,
  ...
  ggsave("../figures/01_Richness.pdf", bg = "white", width = 7, height = 6)
...
soil.vs.lake.abunds <- high.activity.soil.core %>% left_join(mean.soil.abunds.soil.core) %>% 
  mutate(soil_is_source = ifelse(mean_soil_relabund > 1e-3 & RNA.max > 1e-3, T, F)) %>% 
  mutate(Taxon = ifelse(Genus == "unclassified", paste(Family, "sp."), Genus))
soil.vs.lake.abunds %>%   
  ggplot(aes(x = mean_soil_relabund, y = RNA.max)) +
  ...
  annotation_logticks() +
  scale_color_manual(values = my.cols) +
  labs(x = "Mean Soil Relative Abundance", y = "Max Lake RNA \nRelative Abundance") +
  ...
  ggsave("../figures/07_soil-contribs-to-active-water-otus.pdf", bg = "white", height = 6, width = 7)
...
core.metab <- data.frame(
  ...
  dist = design.dna$distance) %>% 
  left_join(metab.resids)
core.metab$total_core_resid <- resid(lm(total_core ~ dist, core.metab)) + mean(core.metab$total_core)
